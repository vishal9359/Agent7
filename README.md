...
```
Clang
 ├── CFG / CallGraph JSON        → direct input to LLM
 └── Description JSON           → validated → stored in RAG

Agent
 ├── Uses CFG as single source of truth for execution
 ├── Uses Description JSON only for naming, labels, explanation
 └── Outputs Mermaid diagrams
```

## 🚫 Hard Rules (Non-Negotiable)

- ✅ Use CLANG library to build CFG (external tool)
- ❌ Never infer execution logic
- ❌ Never modify CFG or CallGraph
- ❌ Never use RAG for control flow
- ❌ Never add branches, loops, or calls not present in CFG
...
- **Validation Rules**:
  - ❌ Mentions conditions not in CFG
  - ❌ Mentions calls not in CallGraph
  - ❌ Claims behavior not supported by execution paths
  - ❌ Uses speculative language ("probably", "might", "seems")
...
- **Validation Rules**:
  ...
  - ❌ Edge count ≠ CFG edges
  - ❌ Missing loop-back edges
  - ❌ Incorrect branch labels
  - ❌ Function calls missing or added
